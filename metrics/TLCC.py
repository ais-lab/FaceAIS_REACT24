...
def calculate_tlcc(pred, sp, seconds=2, fps=25):
    rs = [
        crosscorr(pred, sp, lag, sp.shape[-1])
        for lag in range(-int(seconds * fps - 1), int(seconds * fps))
    ]
    peak = max(rs)
    center = rs[len(rs) // 2]
    ...
    for k in range(speaker.shape[0]):
        ...
        for i in range(pred_item.shape[0]):
            peak, center, offset = calculate_tlcc(
                pred_item[i].float().numpy(), sp_item.float().numpy()
            )
...
def compute_TLCC_mp(pred, speaker, p=8):
    ...
    with mp.Pool(processes=p) as pool:
        # use map
        offset_list += pool.starmap(
            _func, zip(pred.float().numpy(), speaker.float().numpy())
        )
    return torch.mean(torch.Tensor(offset_list)).item()
