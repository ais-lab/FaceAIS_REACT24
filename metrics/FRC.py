...
def compute_FRC(dataset_path, pred, listener_em, val_test="val"):
    pred = pred
        ...
        for k in range(pred.shape[0]):
            ...
            for n_index in range(speaker_neighbour_index_len):
                ...
                ccc = concordance_correlation_coefficient(
                    similar_listener_emotion.numpy(), pred[k, i].numpy()
                )
                ccc_list.append(ccc)
            if len(ccc_list) == 0:
                continue
...
def compute_FRC_mp(dataset_path, pred, em, val_test="val", p=1):
    # pred: N 10 750 25
    # speaker: N 750 25
    if val_test == "val":
